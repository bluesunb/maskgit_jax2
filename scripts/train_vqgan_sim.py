...
def vanilla_d_loss(logits_fake, logits_real = None):
    if logits_real is not None:
        loss_fake = jp.mean(jax.nn.softplus(-loss_fake)) * 2
        loss_real = 0
    else:
...
def train_step(vqgan_state: TrainState, disc_state: TrainState, lpips: LPIPS, 
               batch: jp.ndarray, rng: jp.ndarray, config: LossWeights,
               pmap_axis: str = None):
    ...
    def loss_fn_nll(params):
        ...
        x_recon, q_loss, result = vqgan_state(batch, train=True, rngs=rngs)
    ...
    def loss_fn_generate(params):
        gan_rngs = make_rngs(rng, rng_names['vqgan'])
        disc_rngs = make_rngs(rng, rng_names['disc'])
        x_recon, q_loss, result = vqgan_state(batch, train=True, rngs=gan_rngs)
        gen_loss = disc_state(x_recon, train=True, rngs=disc_rngs)
    ...
    def loss_fn_discriminate(params):
        ...
        x_recon, q_loss, result = vqgan_state(batch, train=True, rngs=g_rngs)
        x_recon = jax.lax.stop_gradient(x_recon)
        
        disc_real = disc_state(batch, train=True, rngs=d_rngs)
        disc_fake = disc_state(x_recon, train=True, rngs=d_rngs)
...
def main(rng,
    ...
    def train_step(state: TrainState, batch: jp.ndarray, rng: jp.ndarray):
        rngs = make_rngs
